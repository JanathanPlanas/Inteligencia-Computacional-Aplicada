...
# Carrega dataset
# ==============================================
Z = np.loadtxt('recfaces_no_pca.dat')
X = Z[:, :-1]
y = Z[:, -1].astype(int)
...
for scaler_name, scaler in scalers.items():
    ...
    for _ in range(Nr):
        X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, train_size=Ptrain, stratify=y)
        start = time.time()
        pl = LogisticRegression(max_iter=1000, multi_class='multinomial', solver='lbfgs')
...
best_results.to_csv(os.path.join(data_folder, 'tabela_resultados.csv'), index=False)
print(f"Resultados salvos em '{os.path.join(data_folder, 'tabela_resultados.csv')}'")
